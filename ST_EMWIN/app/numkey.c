...
//
  { WINDOW_CreateIndirect,   0,        0,                225-150, 130,  95, 130},
...
//  Function                 Text      Id                 Px   Py   Dx   Dy
//
  { FRAMEWIN_CreateIndirect, "Dialog", 0,                 40,  90-90, 140, 115, FRAMEWIN_CF_MOVEABLE},
...
*       MainTask
*/
void MainTask_NUMKEY(void) {
  WM_HWIN hNumPad;

//  GUI_Init();
  WM_SetCallback(WM_HBKWIN, _cbDesktop); //设置背景窗口的回调函数在WM_PAINT中重绘背景
  ...
  WM_SetStayOnTop(hNumPad, 1);         //置位在顶层标志
  while (1) {
		   // GUI_Delay(1000);
